...
## Önemli Notlar
1.  **MovementConfig (Resource) ve Presetler:** **Arda (Godot Core Uzmanı & Muhasebeci)** ve **Elif (Gameplay Programcısı & Bahçıvan)**`ın önerileri doğrultusunda, karakterin temel hareket parametreleri (`WalkSpeed`, `SprintSpeed`, `MaxJumps` vb.) artık Godot `Resource` olarak tanımlanan `MovementConfig` üzerinden yönetilmektedir. Bu `Resource``u `MovementNode``un Inspector panelindeki `Config` slotuna atayarak tüm temel hareket parametrelerini Godot editöründen kolayca ayarlayabilirsiniz. **GÜNCELLEME:** `MovementConfig.cs` içerisindeki `[Export]` değerlerine C# tarafında `Mathf.Clamp` ve `Mathf.Max` ile validasyon eklenerek geçersiz değerlerin sisteme girmesi engellenmiştir. Ayrıca, `addons/virabis_movement/presets/` dizini altında `ninja_preset.tres`, `tank_preset.tres` ve `god_preset.tres` gibi hazır "feel" presetleri oluşturulmuştur. Bu presetler, farklı karakter tipleri veya oyun durumları için hızlıca hareket parametrelerini değiştirmeye olanak tanır.
2.  **"Sudo" Yetkili Movement Decorator:** `MovementNode.cs` artık `IMovementSystem` arayüzünü uygulayan bir `SudoMovementDecorator` üzerinden çalışır. Bu decorator, GodMode, InfiniteJumps, NoClip ve SpeedMultiplier gibi "sudo" yetkilerini ana hareket mantığından bağımsız ve modüler bir şekilde yönetir. Bu sayede, geliştiriciler veya test uzmanları, ana kodu değiştirmeden özel yetenekleri kolayca etkinleştirebilir.
3.  **Mekanik Parametreleri:** Her bir uzman mekaniğin kendine özgü parametreleri (örneğin `ApplyGrapple``daki `springStrength` veya `ApplySlide``daki `friction`) hala GDScript`ten `call()` metodu ile doğrudan `MovementNode``a iletilir. Bu, her mekaniğin anlık duruma göre dinamik olarak ayarlanabilmesini sağlar.
4.  **Sinyaller:** Mekanikler tetiklendiğinde `PlayerController.gd` içindeki ilgili sinyalleri (`jumped`, `slide_started`, `grapple_fired` vb.) `emit` etmeyi unutmayın. Bu, animasyon ve ses sistemleri için gereklidir.
5.  **C# Erişimi:** Tüm bu metodlar `MovementNode.cs` içinde tanımlıdır ve GDScript`ten `call()` metodu ile güvenli bir şekilde çağrılabilir.
